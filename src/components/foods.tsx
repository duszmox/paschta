...
import { Food, News, User } from "@prisma/client";
...
function NewsComp() {
  ...
  const foodQuery = api.food.getSome.useQuery({
    start: currentPage * 10,
  });

  const foodAddMutation = api.food.create.useMutation({
    onMutate: () => {
      setIsMutating(true);
    },
    onSuccess: () => {
      setTitle("");
      setSource("");
      foodQuery.refetch().catch((err) => console.log(err));
    },
    onError: (data) => {
  ...
  useEffect(() => {
    if (foodQuery.data) {
      setFoods(foodQuery.data.foods as Food[]);
      setMaxPage(Math.ceil(foodQuery.data.count / 10));
    }
  }, [foodQuery.data, currentPage]);

  return (
    <div className="my-8">
      {!foodQuery.data || isMutating ? (
            ...
            {foods.length > 0 ? (
              foods.map((food) => (
                <>
                  <div
                    key={food.id}
                    className="mx-2 mb-4 flex w-full max-w-none basis-2/5 flex-col rounded-lg border border-dark bg-orange-50 px-4 py-3"
                  >
              ...
              <div className="text-center">Nincsenek még ételek</div>
            )}
          </div>
        </>
      )}
